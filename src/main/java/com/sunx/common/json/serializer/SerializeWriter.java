...
import com.sunx.common.json.parser.CharTypes;
import com.sunx.common.json.util.Base64;
                ...
                for (int i = 0; i < text.length(); ++i) {
                    ...
                    if (isEnabled(SerializerFeature.BrowserCompatible)) {
                        if (ch == '\b' //
                            ...
                            || ch == '\\') {
                            write('\\');
                            write(CharTypes.replaceChars[(int) ch]);
                            continue;
                        }
        ...
        if (specialCount == 1) {
            ...
            for (int i = textIndex; i < text.length(); ++i) {
                ...
                if (ch < CharTypes.specicalFlags_doubleQuotes.length && CharTypes.specicalFlags_doubleQuotes[ch] //
                    || (ch == '/' && isEnabled(SerializerFeature.WriteSlashAsSpecial))) {
                    buf[bufIndex++] = '\\';
                    buf[bufIndex++] = CharTypes.replaceChars[(int) ch];
                    end++;
                } else {
                    buf[bufIndex++] = ch;
                }
